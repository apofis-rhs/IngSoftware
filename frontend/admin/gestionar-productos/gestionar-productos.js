...
let todosProductos = [];
let todosArticulos = [];
let tabActual      = 'productos';  // 'productos' | 'articulos'
...
async function cargarTodo() {
  ...
  try {
    // Cargar productos y artículos en paralelo
...
function renderLista(lista, tipo) {
  ...
  if (totalEl) totalEl.textContent = `${n} ${tipo === 'productos' ? 'producto' : 'artículo'}${n !== 1 ? 's' : ''}`;

  if (!n) {
    listaEl.innerHTML = '<p style="color:#999;text-align:center;padding:30px">No hay elementos.</p>';
    return;
  }
  ...
  listaEl.innerHTML = lista.map((item, i) => {
    ...
    const dot    = color ? `background:${dotColors[color] || 'var(--color-semaforo-gris)'}` : 'background:var(--color-success)';
    ...
    return `
      <div class="product-item-card fade-in-up" style="animation-delay:${i * 0.04}s">
